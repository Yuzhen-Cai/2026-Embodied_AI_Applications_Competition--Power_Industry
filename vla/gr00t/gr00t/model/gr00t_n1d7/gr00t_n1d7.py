...
def get_backbone_cls(config: Gr00tN1d7Config):
    ...
    # Support both HuggingFace Hub IDs (e.g. "nvidia/Cosmos-Reason2-2B") and
    # local paths (e.g. "/path/to/Cosmos-Reason2-2B") by checking basename too.
    import os

    model_name_or_basename = os.path.basename(os.path.normpath(model_name))
    if (
        "nvidia/Cosmos-Reason2" in model_name
        or "Cosmos-Reason2" in model_name_or_basename
        or "Qwen/Qwen3-VL" in model_name
        or "Qwen3-VL" in model_name_or_basename
    ):
        # We import here as Qwen3Backbone depends on newer transformers versions than the rest of the code.
        from gr00t.model.modules.qwen3_backbone import Qwen3Backbone
