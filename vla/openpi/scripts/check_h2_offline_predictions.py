...
JOINT_GROUPS = {
    "Waist": slice(0, 3),
    "Left arm": slice(3, 10),
    "Right arm": slice(10, 17),
    "Left hand": slice(17, 23),
    "Right hand": slice(23, 29),
}
...
def _episode_metrics(episode_stats: dict[int, dict]) -> list[dict]:
    ...
    for episode_index, stats in sorted(episode_stats.items()):
        ...
        chunk_values = sample_count * 50 * 29
        first_action_values = sample_count * 29
        rows.append(
            {
...
def main(
    ...
    absolute_error_by_step_joint_sum = np.zeros((config.model.action_horizon, 29), dtype=np.float64)
    ...
    for sample_number, (episode_index, index) in enumerate(episode_samples, start=1):
        ...
        if target.shape != (config.model.action_horizon, 29):
    ...
    total_chunk_values = total_samples * config.model.action_horizon * 29
    total_first_action_values = total_samples * 29
    aggregate_metrics = {
